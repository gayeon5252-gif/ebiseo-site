...
> 운영 인수 보완: 2026-09-13. 현재 31개 가이드·48개 사이트맵 URL이며 아래 구조의 29편·46개는 작성 당시 수치다. 시행 예정 표현·등기부 정확성 수정 등 과거 남은 일은 최신 소스와 `이비서-인수인계.md` 상단 정정을 먼저 대조한다. 반복해서 완료 작업을 배분하지 않는다.
>
> 사용자는 매 실행에 두 문서를 읽고 자체 사이트의 수정·검사·배포까지 맡기는 운영을 요청했다. 별도 사무실은 관리 복제본의 실제 파일 수정 → 독립 diff 검수 → 로컬 검사 → main 푸시 → 라이브 확인을 수행하도록 구성하며 실행 근거를 남긴다. 원본 폴더의 미커밋 작업을 덮어쓰지 않는다. GitHub의 매일 09:17 KST 점검은 PC 종료 중에도 실행되지만, AI 사무실의 평일 09~18시 근무에는 PC와 실행기가 필요하다. 둘의 실제 실행 기록을 구분한다.

> 실제 사무실 배포 검증: 2026-09-13 21:48:02 KST, 커밋 `360eb29f`. 관리비 가이드 링크 수정이 직원 배분·실제 수정·독립 검수·main 푸시·라이브 검증까지 완료됐다. 상세 근거와 운영 범위는 인수인계 상단을 따른다.
